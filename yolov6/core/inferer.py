            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        line = (cls, *xywh)
                        id = int(cls.item())
                        xmin = round(xyxy[0].item() * 960/640)
                        xmax = round(xyxy[2].item() * 960/640)
                        ymin = round(xyxy[1].item() * 540/640)
                        ymax = round(xyxy[3].item() * 540/640)

                        with open('sample_submission.csv', 'a') as f_object:
                            writer_object = csv.writer(f_object)
                            writer_object.writerow([id, osp.basename(img_path), names[id], xmax, xmin, ymax, ymin])
                            f_object.close()

                        # myFile = open('sample_submission.csv', 'a')
                        # myList = [id, osp.basename(img_path), names[id], xmax, xmin, ymax, ymin]
                        # writer = csv.writer(myFile)
                        # writer.writerow(myList)
                        # myFile.close()

                        # # print(id, osp.basename(img_path), names[id], xmax, xmin, ymax, ymin)
